#min hero lobby parser

lobby_string ='''          AddObject("generalRoom_floorTile",1926.45,470.2,1,1,0);
...
#basically want to convert this to an XML file...
#isn't perfect but it does the job with a little hand-editing

XML_STRING = '<level width="3051.45" height="2202.15">\n'

lobby_lines = lobby_string.replace(";","").replace(")","").split("\n         AddObject(")

for line in lobby_lines:
  data = line.replace('"',"").split(",")
  XML_STRING += f'  <levelObject spriteName="{data[0]}" xPos="{data[1]}" yPos="{data[2]}" xScale="{data[3]}" yScale="{data[4]}" rotation="{data[5]}"/>\n'

XML_STRING += "</level>"

with open("lobby.xml","w") as file: file.writelines(XML_STRING)